...
async function createAccountRequest(payload: CreateAccountPayload) {
  void payload;
  // TODO: Replace this mock delay with a real backend account creation request.
...
export default function CreateAccountPage({ onFinished, onBackToSignIn }: CreateAccountPageProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!fullName.trim() || !email.trim() || !password.trim() || !confirmPassword.trim()) {
      setError("Please fill out each field to create your account.");
      return;
    }
    if (!emailValid) {
    ...
    setLoading(true);
    try {
      await createAccountRequest({ fullName, email, password, wantsUpdates });
      onFinished();
    } catch {
      setError("We couldn't create your account yet. Please try again.");
    } finally {
      setLoading(false);
    }
  }
