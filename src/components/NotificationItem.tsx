...
const TYPE_META: Record<NotifType, { Icon: React.ComponentType<{ size?: number; className?: string }>; iconBg: string; iconColor: string }> = {
...
export default function NotificationItem({ notification: n, onRead, onAction }: NotificationItemProps) {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${meta.iconBg}`}>
        <meta.Icon size={15} className={meta.iconColor} />
      </div>

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className={["text-sm leading-snug", n.unread ? "font-semibold text-foreground" : "font-medium text-foreground/80"].join(" ")}>
          {n.title}
        </p>
        <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed line-clamp-2">{n.description}</p>
